...
class ADASScenario(object):
    def __init__(self, scenario_series):
        ...
        self.scenario_dict['ego_velocity_time'] = str(scenario_series['自车速度分段持续时间']).split(',')
        self.scenario_dict['ego_trajectory_time'] = str(scenario_series['自车轨迹形态分段持续时间']).split(',')
        ...
        self.scenario_dict['obs_velocity_time'] = str(scenario_series['目标轨迹形态分段持续时间(s)']).split(',')
        self.scenario_dict['obs_heading_angle_rel'] = str(scenario_series['目标物航向角']).split(',')
        ...
        if self.scenario_dict['scenario_name'] in 'PCW':
            self.scenario_dict['ped_initial_position_x'] = str(scenario_series['目标初始x']).split(',')
            self.scenario_dict['ped_initial_position_y'] = str(scenario_series['目标初始y']).split(',')
            ...
            self.scenario_dict['ped_speed_duration_time'] = str(scenario_series['目标速度分段持续时间']).split(',')
            self.scenario_dict['pedT_trail_section_duration_time'] = str(scenario_series['目标轨迹形态分段持续时间']).split(',')
            self.scenario_dict['ped_speed_difference'] = str(scenario_series['自车与目标初始速度差']).split(',')
            self.scenario_dict['ped_direction'] = str(scenario_series['目标正反向标志']).split(',')
