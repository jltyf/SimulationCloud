...
class Scenario(ScenarioGenerator):
    ...
    def road(self, **kwargs):
        ...
        try:
            ...
            uv_coord_info, road_info, left_lane_info, right_lane_info = road_detector_new(enu_file, lane_file, laneid_tuple, polyfit_fig_path)
        except:
    ...
    def scenario(self, **kwargs):
        ...
        init.add_init_action(egoname, xosc.TeleportAction(xosc.WorldPosition(x=positionEgo[0].x, y=positionEgo[0].y, z=0, h=ego_init_h, p=0, r=0)))
        ...
        speedaction = xosc.FollowTrajectoryAction(trajectory, xosc.FollowMode.position,xosc.ReferenceContext.absolute,1,0)
        trigger = xosc.ValueTrigger('drive_start_trigger',0,xosc.ConditionEdge.rising,xosc.SimulationTimeCondition(0,xosc.Rule.greaterThan))
        ...
        sb = xosc.StoryBoard(init, stoptrigger=xosc.ValueTrigger('stop_trigger',0,xosc.ConditionEdge.none,xosc.SimulationTimeCondition(self.sceperiod,xosc.Rule.greaterThan),'stop'))
        sb.add_story(story1)
        ...
        if positionObj:
            ...
            for i in range(len(positionObj)):
                ...
                speedaction2 = xosc.FollowTrajectoryAction(trajectoryM, xosc.FollowMode.position,xosc.ReferenceContext.absolute,1,0)
                ...
                trigger2 = xosc.EntityTrigger("obj-start-trigger", 0, xosc.ConditionEdge.rising, xosc.SpeedCondition(0, xosc.Rule.greaterThan),'Ego')
                event2.add_trigger(trigger2)
                
                if pedflag:
                    pedaction = xosc.FollowTrajectoryAction(trajectoryM, xosc.FollowMode.position,xosc.ReferenceContext.absolute,1,0)
    ...
    def createUDAction(self, **kwargs):
        tree = ET.parse('/home/lxj/Documents/git_project/SimulationCloud/SimulationCloud/Generalization/models/ped_CDATA.xosc')
        root = tree.getroot()
