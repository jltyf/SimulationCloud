...
def get_report(scenario, script_id):
    ...
    # 判断相邻车道目标车ID
    obj_data = scenario.obj_scenario_data[(scenario.obj_scenario_data['object_rel_pos_y'] < -1) & (
                scenario.obj_scenario_data['object_rel_pos_y'] > -5) & (scenario.obj_scenario_data['object_rel_pos_x'] > 0)]  # 横向距离，左为正
    ...
    if CollisionStatus.collision.value not in collision_status_list and (1 < distance < 2.2) and v_diff < 2:
        ...
        score = 0
        evaluate_item = '其他情况，即前方目标车辆变换后，FSRA功能异常，得0分'
    ...
    score_description = '1) 自车减速后稳定跟随目标车辆行驶，且稳定跟车时距为1-2.2s，且减速过程中满足条件“{standard_1}”，即该FSRA系统在有前车切入时，能从定速巡航模式自动变换为跟车模式，且有较好的舒适性，得分100；\n' \
                        ...
                        '3) 其他情况，即前方目标车辆变换后，FSRA功能异常，得0分。'
