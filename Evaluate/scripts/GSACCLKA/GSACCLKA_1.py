...
def get_report(scenario, script_id):
    #lka:
    acc_start_ID = scenario.scenario_data[scenario.scenario_data['longitudinal_acceleration'] < -0.5].index[0]
    ...
    if (1 <= time_distance_min <= 3) and v_diff_acc <= 5 and distance_LKA <= 0 and v_diff_lka <= 2:
        ...
        score = get_v_interpolation(v_diff_lka)*0.5+50
        ...
        score = get_v_interpolation(v_diff_acc)*0.5+50
        ...
        score = get_v_interpolation(v_diff_lka) * 0.5+get_v_interpolation(v_diff_acc)*0.5
